...
public class Simulator {
	...
	/** Pushes a 1-byte character onto the stack. */
	private void pushC(int val) {
		int sp = this.vm.getReg(Machine.SP) - 1;
		this.vm.setReg(Machine.SP, sp);
	...
	/** Pops a 1-byte character from the stack. */
	private byte popC() {
		int sp = this.vm.getReg(Machine.SP);
		byte result = this.vm.loadC(sp);
		this.vm.setReg(Machine.SP, sp + 1);
		return result;
	}
